...
//  nums if the closest non-equal neighbors of i are larger than nums[i]. Adjacent indices i and j are
//   part of the same hill or valley if nums[i] == nums[j].
...
const countHillsAndValleys = nums => {
    let count = 0;
    let left = 0;
    ...
    for (let i = 1; i < nums.length - 1; i++) {
        if (nums[i] !== nums[i + 1]) {
            if (nums[i] > nums[left] && nums[i] > nums[i + 1]) {
                console.log(`Hill at index ${i}: ${nums[i]}`);
                count++;
            } else if (nums[i] < nums[left] && nums[i] < nums[i + 1]) {
                console.log(`Valley at index ${i}: ${nums[i]}`);
                count++;
            }
            left = i;
        }
...
const result = countHillsAndValleys(nums);
console.log("Total hills and valleys:", result);
